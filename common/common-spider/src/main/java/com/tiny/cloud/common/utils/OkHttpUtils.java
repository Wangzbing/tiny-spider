package com.tiny.cloud.common.utils;
...
//         keyManagerFactory.init(keyStore, KEY_STORE_PASSWORD.toCharArray());
     ...
     * @return
     */
    public Response getData(String url,Headers headers) throws Exception {
        //1 构造Request
        Request.Builder builder = new Request.Builder();
        Request request = builder.get().url(url).headers(headers).build();
        //2 将Request封装为Call
        Call call = getOkHttpClient().newCall(request);
        //3 执行Call，得到response
        Response response = null;
